...
function renderCanvas() {
    ...
    ctx.beginPath();
    // novo tamanho das coordenadas de camera
        ...
        for (var j = 0; j < canvas.height; j++) {
            ...
            if (result != 0) {
                ...
                // } else {
                    ...
                    var l = Vec.minus(luz_pontual_p, position); // luz_direcional
                    var v = Vec.minus(position, viewer);
                    // spot
                    var v_pit_cone = Vec.unitary(Vec.minus(position, luz_pontual_p));

                    var d = Vec.module(Vec.minus(position, luz_pontual_p));
                    var att = 1. / (luz_pontual_att.x + luz_pontual_att.y * d + luz_pontual_att.z * d * d);
                    var factor_diff = 0;
                    // && Vec.dot(v_pit_cone, luz_direcional_d) <= angle
